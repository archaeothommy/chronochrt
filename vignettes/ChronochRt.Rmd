...
### Chonological table
This example illustrates the main purpose of the package: facilitate the hasselfree drawing of chronological tables. 
...
If desired, images e.g. of relevant chronological types can be added with the following line: 
```{r}
...
It requires to select a _year_ for the position of each image. This allows the function to place the image, however the actual date will not be visible in the plot. Therefore it should be choosen in a manner, that allows for intuitive display of information. 
...
```
This directly saves a file called "UC-Chronology" in the working directory with a specified size of 15x10 cm.
...
Add some labels, e.g. '12.04.1665 - The "Great Plague of London" begins' and other interesting facts via the following code in different parts of the plot:
```{r}
London_labels <- 
