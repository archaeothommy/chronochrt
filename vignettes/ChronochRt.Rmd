...
### Important note  
In ChronochRt, each _chron_ is independently evaluated. Consequently, all parameters listed above need to be stored for every single _chron_. It is indispensable that start and end date are identical to the preceding and subsequent _chron_ as well as subchrons starting or ending in the same year. Whilst it seems tedious, this feature allows the package to plot interruptions and hiatus in the chronological sequence, like the abandonment of settlements or entire regions (see example below). At the same time it simplifies the structure of the chronological data, as each _chron_ stands for itself.   
...
To indicate a transition period, its `start` and `end` date is given, e.g. `"50/100"` or `"50_100"`. For a distinct date, the two years are the same. For example, `"100/100"` would result in a single horizontal dashed line.  
...
Indeed, their order matters: Diagonal lines are always drawn from the first to the second value and mixing the order will result in different orientations. Dashed lines do not seem to depend on the order that much. However, consistency matters here as well because mixed orders can result in a failure to recognize the true end of the period. Additionally, the vertical position of the labels are based on the first value.  
...
Likewise, image labels are recorded with `add_label_images()`. This function works exactly the same, with taking the path to the image (web address or path to a file on your computer) instead of the text (note that it handles raster and vector files): 
...
However, it is strongly recommended to use the geoms provided by ChronochRt for the design of more complex design (see chapter "ChronochRt for advanced R users"). 
...
Set the general sedign with the `theme_chronochrt()`: 
```{r advanced2, echo=TRUE, fig.align='center', fig.width=10, message=FALSE, out.width="100%"}
plot + theme_chronochrt()
```

However, some fine tuning is still needed. By default, the axes are expanded. Additionally, the first example indicates, that the default facetting of ggplot2 is also not optimal: 
