...
## The data set 
Chronological data are stored in a table. Each row repersents a _chron_ (a chronological unit of any kind). The columns store the information of these chrons. The basic information of a _chron_ is:  
...
These six variables are essential for the package and therefore cannot be renamed. However, when you import or convert a data set, you will have to indicate the corresponding columns as their original name is not evaluated during import/creation or conversion. It is possible to store additional variables in the chrons, such as information to change e.g. the position and angle of the _chron_'s names or additional information, useful for further work with the data set.
...
### How does it work? 
Each region is plotted independently. Within a region, all _chrons_ with `add = FALSE` will be evenly scaled according to the maximum `level`, e.g. in region A and the `add = TRUE`-column is 5, hence, all _chrons_ will be 1/5 wide. Similarly in region B, the maximum `level`is 3, consequently all _chrons_ are 1/3 wide. This essentially transform the x-axis into percentages, allowing to easily add additional labels or customise the the position and angle of the _chrons'_ names. Likewise, all _chrons_ in the `add = TRUE`-cloumn are displayed with their x-value shifted by 1, i.e. they are plotted between 1 and 2.  
