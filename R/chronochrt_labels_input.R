...
#' placed centered on the \code{year} given. Text in labels can be wrapped by
...
#' @param position A number or a numeric vector with the horiontal position(s)
...
add_label_text <- function(data, region, year, position = 0.9, label, new = FALSE, ...)
{
  if (new == FALSE) {
    ...
    } else {stop("The object ", substitute(data) , " does not exist.")
    }
  } else {
...
#'   path(s) or weblinks to the image files.
#' @param new Logical operator. If \code{TRUE}, a new data set will be created.
#'   If \code{FALSE}, the default, the input will be added to an existing data
#'   set.
#' @param ... Further arguments or columns to include in or additional arguments
