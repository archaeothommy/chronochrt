...
#'
#' \code{start} and \code{end} of neighbouring chronological units must be fit
#' to get a good plotting results, as well as the start date of e.g. the top
#' chronological unit and its respective oldest sub-units. Dates in BCE must
#' provided as negative data. Currently, only years can be handled (i.e. 2020
#' but not 20.20.2020).
#'
#' Is \code{start} and \code{end} dates are not certain or the change between
#' chronological units is regarded a period, dates must be given as character
#' string in the format \code{"1000/2000"}. The order does not matter. But also
#' here, be consistent in order and in matching \code{start} and \code{end}
#' dates to avoid unclean borders in the plot.
#'
