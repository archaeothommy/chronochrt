...
plot_chronochrt <- function(data, labels_text = NULL, labels_image = NULL,
                            year_lim = NULL, axis_title = "Years",
                            minimal = FALSE, size_text = 6, height_image = 2,
                            size_line = 0.5, fill_chron = "white",
                            color_chron = "black", color_label = "black",
                            line_break = 10, filename = NULL, plot_dim,
                            background = NULL, ...) {

  if (is.data.frame(data)) {

    if (!("region" %in% names(data))) {
      stop("Wrong input format: The column `region` in ",
           substitute(data), " does not exist.")
    }
    if (!("name" %in% names(data))) {
      stop("Wrong input format: The column `name` in ",
           substitute(data), " does not exist.")
    }
    if (!("start" %in% names(data))) {
      stop("Wrong input format: The column `start` in ", substitute(data),
           " does not exist.")
    }
    if (!("end" %in% names(data))) {
      stop("Wrong input format: The column `end` in ", substitute(data),
           " does not exist.")}
    if (!("level" %in% names(data))) {
      stop("Wrong input format: The column `level` in ", substitute(data),
           " does not exist.")}
    if (!("add" %in% names(data))) {
      stop("Wrong input format: The column `add` in ", substitute(data),
           " does not exist.")}

    if (!all(is.character(data$region) | is.factor(data$region),
    ...
    if (!all(round(data$level) == data$level)) {
      stop("Wrong input format: level must contain only whole numbers
           (1, 2, 3, ...)")
    }
  } else {
  ...
  if (!is.null(labels_text)) {
    if (is.data.frame(labels_text)) {
      if (!("region" %in% names(labels_text))) {
        stop("Wrong input format: The column `region` in ",
             substitute(labels_text), " does not exist.")
      }
      if (!("year" %in% names(labels_text))) {
        stop("Wrong input format: The column `year` in ",
             substitute(labels_text), " does not exist.")
      }
      if (!("label" %in% names(labels_text))) {
        stop("Wrong input format: The column `label` in ",
             substitute(labels_text), " does not exist.")
      }
      if (!("position" %in% names(labels_text))) {
        stop("Wrong input format: The column `position` in ",
             substitute(labels_text), " does not exist.")
      }

      if (!all(is.character(labels_text$region) | is.factor(labels_text$region),
               is.numeric(labels_text$year),
               is.character(labels_text$label),
               is.numeric(labels_text$position)
      )
      ) {
        stop("One or more columns of the text label data contain incompatible
             data. Data must be strings (region, label) and numeric (year,
             position).")
      }
    } else {
      stop("Wrong input format: ", substitute(labels_text) , " must be a data
           frame or tibble.")
    }
  }
  ...
  if (!is.null(labels_image)) {
    if (is.data.frame(labels_image)) {
      if (!("image_path" %in% names(labels_image))) {
        stop("Wrong input format: The column `region` in ",
             substitute(labels_image), " does not exist.")
      }
      if (!("region" %in% names(labels_image))) {
        stop("Wrong input format: The column `region` in ",
             substitute(labels_image), " does not exist.")
      }
      if (!("year" %in% names(labels_image))) {
        stop("Wrong input format: The column `year` in ",
             substitute(labels_image), " does not exist.")
      }
      if (!("position" %in% names(labels_image))) {
        stop("Wrong input format: The column `position` in ",
             substitute(labels_image), " does not exist.")
      }

      if (!all(
        is.character(labels_image$region) | is.factor(labels_image$region),
        is.numeric(labels_image$year), is.character(labels_image$image_path),
        is.numeric(labels_image$position)
      )
      ) {
        stop("One or more columns of the image label data contain incompatible
             data. Data must be strings (region, label, image_path) and numeric
             (year, position).")
      }
    } else {
      stop("Wrong input format: ", substitute(labels_image) , " must be a data
           frame or tibble.")
    }
  }

  if (!is.null(year_lim) && any(!is.numeric(year_lim), length(year_lim) != 2)) {
    stop(
      "Wrong input format: ", year_lim, " must be a numberic vector of length 2."
    )
  }
  if (!is.character(axis_title)) {
    stop("Wrong input format: ", axis_title, " must be a character string.")
  }
  if (!is.numeric(size_text) || length(size_text) != 1) {
    stop("Wrong input format: ", size_text, " must be a number.")
  }
  if (!any(is.character(fill_chron),
           is.numeric(fill_chron)) || length(fill_chron) != 1) {
    fill_chron <- "white"
  }
  if (!any(is.character(color_chron),
           is.numeric(color_chron)) || length(color_chron) != 1) {
    color_chron <- "black"
  }
  if (!any(is.character(color_label),
           is.numeric(color_label)) || length(color_label) != 1) {
    color_label <- "black"
  }
  if (!is.numeric(height_image) || length(height_image) != 1) {
    stop("Wrong input format: ", height_image, " must be a number.")
  }
  if (!is.logical(minimal)) {
    stop("Wrong input format: ", minimal, " must be 'TRUE' or 'FALSE'.")
  }
  if (!is.numeric(size_line) || length(size_line) != 1) {
    stop("Wrong input format: ", size_line, " must be a number.")
  }
  if (!is.numeric(line_break) || length(line_break) != 1) {
    stop("Wrong input format: ", line_break, " must be a number.")
  }
  if (!is.null(filename)) {
    if (!is.character(filename)) {
      stop("Wrong input format: ", filename, " must be a character string.")
    }
    if (!dir.exists(dirname(filename))) {
      stop("The directory ", dirname(filename), " does not exist.")
    }
    if (!is.null(plot_dim)) {

      if (!plot_dim[3] %in% c("in", "cm", "mm")) {
        stop("This unit is not supported. Only the following units are support:
             mm, cm, in.")
      }
  ...
  if(!is.null(background)) {
    if (length(background) != 2) {
      stop("Wrong input format:", background, " is not a vector of length 2.")
    }
  ...
  plot <- ggplot2::ggplot() +
    geom_chronochRt(data = data, ggplot2::aes(region = .data$region,
                                              name = .data$name,
                                              start = .data$start,
                                              end = .data$end,
                                              level = .data$level,
  ...
  if (!is.null(labels_image)) {
    plot <- plot + geom_chronochRtImage(
      data = labels_image,
      ggplot2::aes(
  ...
  plot <- plot +
    ...
    ggplot2::facet_grid(cols = ggplot2::vars(.data$region), scales = "free_x",
                        space = "free_x", labeller = ggplot2::label_wrap_gen(
                          width = line_break
                        )
    ) +
    theme_chronochrt() +
    ggplot2::theme(axis.text = ggplot2::element_text(
      size = size_text*0.8*72.27/25.4
    ),
    axis.title = ggplot2::element_text(
      size = size_text*72.27/25.4,
      face="bold"
    ),
    strip.text.x = ggplot2::element_text(
      size = size_text*1.25*72.27/25.4,
      face="bold"
    )
    )
