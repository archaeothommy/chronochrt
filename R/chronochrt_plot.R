...
plot_chronochrt <- function(
  ...
  if (is.data.frame(data)) {
    ...
    if (!all(
      is.character(data$region) | is.factor(data$region),
  ...
  if (!is.null(labels_text)) {
    if (is.data.frame(labels_text)) {
      ...
      if (!all(
        is.character(labels_text$region) | is.factor(labels_text$region),
      ...
      stop("Wrong input format: ", substitute(labels_text),
  ...
  if (!is.null(labels_image)) {
    if (is.data.frame(labels_image)) {
      ...
      stop("Wrong input format: ", substitute(labels_image),
           " must be a data frame or tibble.")
    }
  }
  ...
  if (!is.null(background)) {
    plot <- plot +
      ggplot2::theme(
        panel.background = ggplot2::element_rect(fill = bg_fill),
