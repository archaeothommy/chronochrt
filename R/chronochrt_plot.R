...
plot_chronochrt <- function(data,
  ...
  if (is.data.frame(data)) {
    ...
    if (!("start" %in% names(data))) {
      stop("Wrong input format: The column `start` in ",
           substitute(data),
           " does not exist.")
    }
    if (!("end" %in% names(data))) {
      stop("Wrong input format: The column `end` in ",
           substitute(data),
           " does not exist.")
    }
    if (!("level" %in% names(data))) {
      stop("Wrong input format: The column `level` in ",
           substitute(data),
           " does not exist.")
    }
    if (!("add" %in% names(data))) {
      stop("Wrong input format: The column `add` in ",
           substitute(data),
           " does not exist.")
    }
    ...
    if (!all(
      ...
      stop(
        "One or more columns of the data set contain incompatible data. Data must be:
        strings (region, name),
        numbers (start, end),
        whole numbers (level),
        and logical (add)."
  ...
  if (!is.null(labels_text)) {
    if (is.data.frame(labels_text)) {
      if (!("region" %in% names(labels_text))) {
        stop(
          "Wrong input format: The column `region` in ",
          substitute(labels_text),
          " does not exist."
        )
      }
      if (!("year" %in% names(labels_text))) {
        stop(
          "Wrong input format: The column `year` in ",
          substitute(labels_text),
          " does not exist."
        )
      }
      if (!("label" %in% names(labels_text))) {
        stop(
          "Wrong input format: The column `label` in ",
          substitute(labels_text),
          " does not exist."
        )
      }
      if (!("position" %in% names(labels_text))) {
        stop(
          "Wrong input format: The column `position` in ",
          substitute(labels_text),
          " does not exist."
        )
      }
      ...
      if (!all(
        ...
        stop(
          "One or more columns of the text label data contain incompatible data.
          Data must be strings (region, label) and numeric (year, position)."
        )
      }
    } else {
      stop("Wrong input format: ",
  ...
  if (!is.null(labels_image)) {
    if (is.data.frame(labels_image)) {
      ...
      if (!all(
        is.character(labels_image$region) |
          is.factor(labels_image$region),
        is.numeric(labels_image$year),
        is.character(labels_image$image_path),
        is.numeric(labels_image$position)
      )) {
        stop(
          "One or more columns of the image label data contain incompatible data.
          Data must be strings (region, label, image_path) and numeric (year, position)."
        )
      }
    } else {
      stop("Wrong input format: ",
  ...
  if (!is.null(year_lim) &&
        any(!is.numeric(year_lim), length(year_lim) != 2)) {
    stop("Wrong input format: ",
         year_lim,
         " must be a numberic vector of length 2.")
  }
  ...
  if (!is.numeric(size_text) ||
        length(size_text) != 1) {
    stop("Wrong input format: ", size_text, " must be a number.")
  }
  if (!any(is.character(fill_chron), is.numeric(fill_chron)) ||
        length(fill_chron) != 1) {
    fill_chron <- "white"
  }
  if (!any(is.character(color_chron), is.numeric(color_chron)) ||
        length(color_chron) != 1) {
    color_chron <- "black"
  }
  if (!any(is.character(color_label), is.numeric(color_label)) ||
        length(color_label) != 1) {
    color_label <- "black"
  }
  if (!is.numeric(height_image) ||
        length(height_image) != 1) {
  ...
  plot <- plot +
    ...
    ggplot2::facet_grid(
      ...
      scales = "free_x",
      space = "free_x",
      labeller = ggplot2::label_wrap_gen(width = line_break)
    ) +
    theme_chronochrt() +
    ggplot2::theme(
      axis.text = ggplot2::element_text(size = size_text * 0.8 * 72.27 / 25.4),
      axis.title = ggplot2::element_text(size = size_text *
                                           72.27 / 25.4, face = "bold"),
      strip.text.x = ggplot2::element_text(size = size_text *
                                             1.25 * 72.27 / 25.4, face = "bold")
    )
